        ...
        for event, elem in ET.iterparse(input_file):
            if event == "end":
                try:
                    ...
                    pass
                pbar.update(1)
    assert len(d["PostId"]) == len(d["UserId"]) and len(d["UserId"]) == len(d["Score"]) and len(d["Score"]) == len(
        d["CreationDate"]) and len(d["Score"]) == len(d["Text"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input", default="../dataset/ai/Comments.xml",
                        help="input: */Comments.xml, output: */Comments.csv")
