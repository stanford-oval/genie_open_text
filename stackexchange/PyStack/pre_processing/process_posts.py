...
def process_question_tags(output_dir, Questions):
    ...
    sprint(output_dir, "pystack_analysis.log",
           "# tags per question: %0.4f" % (sum([len(x) for _, x in question_tags.items()]) * 1.0 / len(question_tags)))
...
def posts_processing(file_name):
    ...
    tag_count = sum(1 for event, _ in ET.iterparse(file_name) if event == "end")
